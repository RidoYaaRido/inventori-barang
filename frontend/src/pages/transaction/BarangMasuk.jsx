...
export default function BarangMasuk() {
  ...
  async function handleSubmit(event) {
    ...
    try {
      const response = await stockInApi.create(form)
      const created = getApiData(response, response.data)

      if (file && created?.id) {
        const formData = new FormData()
        formData.append('attachment', file)
        await stockInApi.upload(created.id, formData)
      }

      setSuccess(file ? 'Barang masuk berhasil disimpan dan bukti berhasil diupload.' : 'Barang masuk berhasil disimpan.')
      setForm(initialForm)
      setFile(null)
    } catch (err) {
      setError(getApiMessage(err, 'Gagal menyimpan barang masuk.'))
    } finally {
      setSubmitting(false)
    }
  }
  ...
  return (
    <section className="container-fluid py-4">
      <h2>Barang Masuk</h2>
      <p className="text-muted">Input transaksi barang masuk dari data backend.</p>
          ...
          {loading ? (
            ...
            <form className="row g-3" onSubmit={handleSubmit}>
              ...
              <div className="col-md-3">
                ...
                <input className="form-control" name="received_at" type="datetime-local" value={form.received_at} onChange={updateField} />
              </div>
              <div className="col-md-6">
